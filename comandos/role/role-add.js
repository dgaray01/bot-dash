...
export default class extends Command {
  ...
  async run(message, args) {
    ...
    if (!user)
      return message.channel.send("Debes mencionar a un usuario válido.");
    if (user == owner)
      return message.channel.send("No puedes añadir roles a ese usuario");
    ...
    await user.roles.add(role.id);
    return message.channel.send(
      `El rol **${role.name}** ha sido agregado correctamente al usuario **${user.displayName}** | Dado por: **${message.member.displayName}**`
    );
  }
}
