...
export default class extends Command {
  constructor(options){
    super(options)
    this.usage = "lvl-color <HEX COLOR>";
  }
  async run(message, args){
    ...
    if(!args.join("").match(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/g)) return message.channel.send(error)
         ...
         let update = `UPDATE userLvl SET color = '${args.join("")}' WHERE id = ${message.author.id}`;

         db.run(update, function(err) {      
          if (err) return console.error(err.message)
          message.channel.send("Color de TextBox cambiado a " + args.join(""))
