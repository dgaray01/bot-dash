...
export default class extends Command {
  ...
  async run(message, args){
    ...
    if(!usuario) return message.channel.send("Debiste mencionar un usuario")
    
    const embed = new Discord.MessageEmbed()
    .setAuthor(`¿Estas seguro que quieres ELIMINAR la database del usuario ${message.mentions.users.first()}?`)
...
case 0: 
    ...
    let SQLDelete = `DELETE FROM userLvl WHERE id = '${usuario.id}'`;

db.run(SQLDelete, function(err) {
    if (err) return console.error(err.message)
})
    
    message.channel.send("Usuario reiniciado")
